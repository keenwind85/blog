...
순시미네를 만들 때는 [[순시미네 1편 - 기획자가 처음으로 앱을 직접 만들기로 하다|"기획자가 직접 만들 수 있을까?"]]라는 질문으로 시작했다. 백엔드 리팩토링에서는 [[백엔드 도전기 1편 - 기획자가 왜 백엔드를 뜯어고치게 되었나|"기획자가 백엔드를 건드려도 되나?"]]라는 질문이었다. 맞고에서는 **"기획자가 게임을 만들 수 있을까?"**였다.
